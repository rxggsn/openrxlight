package cn.ggsn.rxlight.ai.claw.impl.lark;
...
import com.lark.oapi.service.contact.v3.enums.ListUserDepartmentIdTypeEnum;
...
import cn.ggsn.rxlight.ai.claw.impl.lark.card.CallbackVariableTransformer;
import cn.ggsn.rxlight.ai.claw.impl.lark.card.CardActionHandler;
import cn.ggsn.rxlight.ai.claw.impl.lark.card.CardTemplateCache;
import cn.ggsn.rxlight.ai.claw.impl.lark.model.CardMessage;
import cn.ggsn.rxlight.ai.claw.impl.lark.model.FileMessage;
import cn.ggsn.rxlight.ai.claw.impl.lark.model.ImageMessage;
import cn.ggsn.rxlight.ai.claw.impl.lark.model.LocationMessage;
import cn.ggsn.rxlight.ai.claw.impl.lark.model.CardMessage.CardData;
import cn.ggsn.rxlight.ai.claw.impl.lark.model.RichTextPost.PostContent;
import cn.ggsn.rxlight.ai.claw.impl.lark.model.RichTextPost;
import cn.ggsn.rxlight.ai.claw.impl.lark.model.TextMessage;
...
@Slf4j
public class LarkBot extends ClawBot {
    ...
    private static class LarkMessageParser {
        public static RxLightChatMessage parseP2MessageReceiveV1(P2MessageReceiveV1 event) {
            var eventData = event.getEvent();
            String openId = eventData.getSender().getSenderId().getOpenId();
            var account = QuarkusTransaction.requiringNew()
                    .call(() -> {
                        return (Account) Account
                                .getAccountByExternalAccount(openId, ExternalAccountType.FEISHU, AccountType.OPERATOR)
    ...
    public LarkBot(int agentId,
            ...
            throws IOException {
        super(new LarkContext(redis, agentId));
                ...
                .eventHandler(EventDispatcher
                        ...
                        .onP2MessageReceiveV1(new P2MessageReceiveV1Handler() {
                            ...
                            public void handle(P2MessageReceiveV1 event) throws Exception {
                                push(LarkMessageParser.parseP2MessageReceiveV1(event));
                            }
                        })
                        .onP2BotMenuV6(new ApplicationService.P2BotMenuV6Handler() {
                            @Override
                            public void handle(P2BotMenuV6 event) throws Exception {
                                var account = QuarkusTransaction.requiringNew().call(() -> {
                                    return Account.getAccountByExternalAccount(
                                    ...
                                    openRxLightAccounts.stream().findFirst().ifPresent(externalAccount -> {
                                        try {
                                            ...
                                            createMessage(RxLightChatMessage
                                                    ...
                                                    .callback(Callback.builder().type(CallbackType.CUSTOM_CMD)
                        ...
                        .onP2UserCreatedV3(new P2UserCreatedV3Handler() {
                            ...
                            public void handle(P2UserCreatedV3 event) throws Exception {
                                ...
                                var account = QuarkusTransaction.requiringNew().call(() -> Account
                                        ...
                                        .orElseGet(() -> {
                                            ...
                                            if (Boolean.TRUE.equals(user.getIsTenantManager())) {
                                                ...
                                                acc.addRoleType(RoleType.OPERATOR);
                                            }
    ...
    private void initializeAccounts() {
        var response = Future.await(this.redis.get("claw:lark:accounts:sync:flag"));
        ...
        try {
            ...
            var resp = this.client
                    .contact()
                    .v3()
                    .department()
                    .list(ListDepartmentReq
                            .newBuilder()
                            .departmentIdType(ListDepartmentDepartmentIdTypeEnum.OPEN_DEPARTMENT_ID)
                            .userIdType(ListDepartmentUserIdTypeEnum.OPEN_ID)
                            .fetchChild(true)
                            .pageSize(pageSize)
                            .build());
            if (!resp.success()) {
                log.error("Failed to fetch departments from Lark: {}", resp.getMsg());
                return;
            }

            if (resp.getData().getItems() != null) {
                for (Department items : resp.getData().getItems()) {
                ...
                while (userHasMore) {
                    var userResp = this.client.contact()
                            ...
                            .list(ListUserReq
                                    ...
                                    .departmentIdType(
                                            ListUserDepartmentIdTypeEnum.OPEN_DEPARTMENT_ID)
                                    .userIdType(ListUserUserIdTypeEnum.OPEN_ID)
                                    .pageSize(pageSize)
                ...
                while (subDepartmentsHasMore) {
                    var subDepartmentResp = this.client
                            ...
                            .list(ListDepartmentReq
                                    .newBuilder()
                                    .departmentIdType(
                                            ListDepartmentDepartmentIdTypeEnum.OPEN_DEPARTMENT_ID)
                                    .userIdType(ListDepartmentUserIdTypeEnum.OPEN_ID)
                                    .parentDepartmentId(department.getOpenDepartmentId())
                                    .pageSize(pageSize)
        ...
        Future.await(this.redis.set(Lists2.of("claw:lark:accounts:sync:flag", "1"))
                .map(r -> null)
                .onFailure(ex -> {
    ...
    private List<CreateMessageReqBody> transformToCreateMessageReq(RxLightChatMessage msg) throws Exception {
        var feishuAccount = Account.getByAccountId(msg.getUserId(), AccountType.OPERATOR)
