...
public final class BeanUtils {
    ...
    public static <T> List<Map<String, Object>> beansToMaps(List<T> beans) {
        if (Lists2.isEmpty(beans)) {
            return Collections.emptyList();
        }

        return Lists2.map(beans, BeanUtils::beanToMap);
    }
    ...
    public static <T> List<T> mapsToBeans(List<? extends Map<String, ?>> maps, Class<T> clazz) {
        if (Lists2.isEmpty(maps)) {
            return Collections.emptyList();
        }
        return Lists2.map(maps, e -> mapToBean(e, clazz));
    }

    public static List<Field> getAllFields(Class<?> clazz) {
        List<Field> fields = Lists2.empty();
        while (clazz != null) {
