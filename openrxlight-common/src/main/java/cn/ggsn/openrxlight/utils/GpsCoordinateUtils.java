...
public class GpsCoordinateUtils {
    ...
    //简单判断经纬度是否在国内
    // 中国边界经纬度，简化为四个角：西北和东南角
    private static final double CHINA_NORTH_LAT = 53.5;
    private static final double CHINA_EAST_LON = 135.0;
    private static final double CHINA_SOUTH_LAT = 3.5;
    private static final double CHINA_WEST_LON = 73.666667;

    public static boolean isInChina(double latitude, double longitude) {
        return latitude >= CHINA_SOUTH_LAT && latitude <= CHINA_NORTH_LAT &&
                longitude >= CHINA_WEST_LON && longitude <= CHINA_EAST_LON;
    }

    /*public static boolean isLocationOutOfChina(double latitude, double longitude) {
    ...
    public static double[] calWGS84toGCJ02(double latitude, double longitude) {
        ...
        return new double[]{subScale(retLat), subScale(retLon)};
    }
    ...
    //截取经纬度长度为6位
    private static double subScale(double value) {
