 ...
 *     微信开放平台全局返回码.
 *     参考文档：<a href=
"https://developers.weixin.qq.com/doc/oplatform/Return_codes/Return_code_descriptions_new.html">开放平台全局返回码</a>
 * </pre>
 *
 * @author Lam Jerry
 */
...
public enum WxOpenErrorMsgEnum {
    ...
    /**
     * 获取 access_token 时 AppSecret 错误，或者 access_token 无效。请开发者认真比对 AppSecret
     * 的正确性，或查看是否正在为恰当的公众号调用接口 invalid credential, access_token is invalid or not
     * latest
     */
    ...
    /**
     * 不合法的请求字符，不能包含 \\uxxxx 格式的字符 invalid charset. please check your request, if
     * include \\uxxxx will create fail!
     */
    ...
    /**
     * not completely ok, err_item will return location_id=-1,check your
     * required_fields in json.
     */
    ...
    /**
     * invalid DateInfo, Make Sure OldDateInfoType==NewDateInfoType &&
     * NewBeginTime<=OldBeginTime && OldEndTime<= NewEndTime
     */
    CODE_40100(40100,
    ...
    /**
     * invalid card id! 1,card status must verify ok; 2,this card must have
     * location_id
     */
    ...
    /**
     * invalid user-card status! Hint: the card was given to user, but may be
     * deleted or expired or set unavailable !
     */
    CODE_40127(40127,
    ...
    /**
     * invalid not supply bonus, can not change card_id which supply bonus to be not
     * supply
     */
    ...
    /**
     * 1)maybe share_card_list.num or consume_share_self_num too big; 2)maybe
     * card_id_list also has self-card_id;3)maybe card_id_list has many different
     * card_id;4)maybe both consume_share_self_num and share_card_list.num bigger
     * than 0
     */
    CODE_40144(40144,
    ...
    /**
     * invalid update! Can not both set PayCell and CenterCellInfo(include:
    ...
    /**
     * access_token 超时，请检查 access_token 的有效期，请参考基础支持 - 获取 access_token 中，对
     * access_token 的详细机制说明 access_token expired
     */
    ...
    /**
     * 用户修改微信密码， accesstoken 和 refreshtoken 失效，需要重新授权 access_token and refresh_token
     * exception
     */
    ...
    /**
     * some argument may be out of length limit! please check document and request
     * json!
     */
    ...
    /**
     * can not set share_friends=true because has no Abstract Or Text_Img_List has
     * no img Or image url not valid
     */
    CODE_45043(45043,
    ...
    /**
     * 下发输入状态，需要之前30秒内跟用户有过消息交互 need sending message to user, or recving message
     * from user in the last 30 seconds before typing
     */
    ...
    /**
     * part or whole of the requests from the very app is temporary blocked by
     * supervisor
     */
    ...
    /**
     * this api have been not supported since 2020-01-11 00:00:00, please use new
    ...
    /**
     * data format error, do NOT use json unicode encode (\\uxxxx\\uxxxx), please
     * use utf8 encoded text!
     */
    CODE_47002(47002,
    ...
    /**
     * 当天提交页面数达到了配额上限，请明天再试 submit pages count reach daily limit, please try
     * tomorrow
     */
    ...
    /**
     * api 接口被封禁，请登录 mp.weixin.qq.com 查看详情 api forbidden for irregularities, view
     * detail on mp.weixin.qq.com
     */
    ...
    /**
     * forbid to modify the material， please see more information on
     * mp.weixin.qq.com
     */
    ...
    /**
     * api 功能未授权，请确认公众号/小程序已获得该接口，可以在公众平台官网 - 开发者中心页中查看接口权限 access clientip is not
     * registered
     */
    ...
    /**
     * invalid total cost, it must be smaller than the sum of product and shipping
     * cost
     */
    ...
    /**
     * invalid category_index OR category.title is empty OR is_banner but
     * has_category_index
     */
    ...
    /**
     * Invalid Args! page_id.size!=0 but all==true, or page_id.size==0 but
     * all==false.
     */
    ...
    /**
     * 没有新增域名，请确认小程序已经添加了域名或该域名是否没有在第三方平台添加 no domain to modify after filtered,
     * please confirm the domain has been set in miniprogram or open
     */
    ...
    /**
     * 小程序类目信息失效（类目中含有官方下架的类目，请重新选择类目） some category you choose is no longger
     * supported, please choose other category
     */
    ...
    /**
     * 小程序已使用 api navigateToMiniProgram，请声明跳转 appid 列表后再次提交 navigatetominiprogram
     * appid list empty
     */
    ...
    /**
     * 该版本不能回退，可能的原因：1:无上一个线上版用于回退 2:此版本为已回退版本，不能回退 3:此版本为回退功能上线之前的版本，不能回退 forbid
     * revert this version release
     */
    ...
    /**
     * 尚未设置小程序业务域名，请先在第三方平台中设置小程序业务域名后在调用本接口 open's webview domain is null! Need to
     * set open's webview domain first!
     */
    ...
    /**
     * 不存在该插件 appid plugin appid is no in the plugin list, make sure plugin appid is
     * right
     */
    ...
    /**
     * code参数无效，请检查code长度以及内容是否正确；注意code_type的值不同需要传的code长度不一样 enterprise code
     * invalid
     */
    ...
    /**
     * invalid ssid, can not include none utf8 characters, and should be between 1
     * and 32 bytes
     */
    ...
    /**
     * 快递侧逻辑错误，详细原因需要看 delivery_resultcode, 请先确认一下编码方式，python建议 json.dumps(b,
     * ensure_ascii=False)，php建议 json_encode($arr, JSON_UNESCAPED_UNICODE) Delivery
     * side error
     */
    CODE_9300501(9300501,
    ...
    /**
     * Token 不正确 invalid token, can't decryption or decryption result is different
     * from the plaintext
     */
    ...
    /**
     * update_type 不正确,请使用"bind" 或者“unbind” invalid update_type, please use [bind]
     * or [unbind]
     */
